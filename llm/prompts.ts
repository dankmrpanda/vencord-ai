...
3. \`search_messages\`: Search Discord's server-side index and local channel history for keywords, authors, dates, or media attachments (has: link, file, image, etc.).
...
### Rules & Guidelines:
- **Search Strategy**: Start by querying relevant keywords, authors, or attachment filters (\`has: 'link'\`, \`has: 'file'\`, \`has: 'image'\`). If looking for recent channel events or if server search yields few results, check \`fetch_recent_messages\`. If you find a relevant message, call \`fetch_surrounding_messages\` on that message ID to get the complete conversation.
...
export const AGENT_TOOLS: ToolDefinition[] = [
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Searches message history using Discord server-side search index and local channel context. Supports keywords, authors, attachments, and specific channel filters.',
      parameters: {
        type: 'object',
        properties: {
          query: {
            type: 'string',
            description: 'The search keywords or phrase to look for (can be omitted or left blank when searching by media type with has).',
          },
          channel_id: {
            type: 'string',
            description: 'Optional specific channel ID to search in (must be within allowed scope, defaults to current channel if querying current channel).',
          },
          author_id: {
            type: 'string',
